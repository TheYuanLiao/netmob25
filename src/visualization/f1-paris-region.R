...
paris_ctr_box <- c(2.2500099847,48.8119413303,2.4184473464,48.9039038704)
...
# Compute bbox center
cx <- (bbox2plot$xmin + bbox2plot$xmax) / 2
cy <- (bbox2plot$ymin + bbox2plot$ymax) / 2
# 2) Plot
g1 <- ggmap(paris_basemap) +
  ...
  geom_sf(data = lines, aes(color = Mode),
          size = 0.02, alpha = 0.3, inherit.aes = FALSE, show.legend = TRUE) +
  scale_color_manual(
    name = "Public transit",
...
zoom_map <- get_stadiamap(
  bbox = c(left  = paris_ctr_box[1],
           ...
           top   = paris_ctr_box[4]),
  maptype = "stamen_toner_lines", zoom = 14
)
...
g_zoom <- ggmap(zoom_map) +
  ...
  #         alpha = 0.8, inherit.aes = FALSE, show.legend = FALSE) +
  geom_sf(data = lines, aes(color = Mode),
          size = 0.01, alpha = 0.3, inherit.aes = FALSE, show.legend = FALSE) +
  scale_color_manual(
    values = c(Metro = "#D81B60", Tram = "#1E88E5", Rail = "#ddcc77"),
    guide = "none"   # <--- kill the legend
  ) +
  geom_point(data = df.l1, aes(x = lon, y = lat),
             color = "darkgreen", size = 0.5, alpha = 0.5,
             inherit.aes = FALSE, show.legend = FALSE) +
  coord_sf(
    xlim = c(paris_ctr_box[1], paris_ctr_box[3]),
    ylim = c(paris_ctr_box[2], paris_ctr_box[4]),
    expand = FALSE
  ) +
  annotation_scale(location = "bl", width_hint = 0.3, text_cex = 0.5) +
  annotation_north_arrow(
...
G <- ggarrange(g1, g_zoom, ncol = 2, nrow = 1, labels=c('', ''))
ggsave(filename = "figures/paris_region.png", plot = G, bg='white',
